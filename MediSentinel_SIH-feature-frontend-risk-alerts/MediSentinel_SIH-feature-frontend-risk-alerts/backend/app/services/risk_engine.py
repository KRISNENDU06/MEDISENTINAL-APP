...
class RiskEngine:
    ...
    def assess_area(self, area: Area, assessed_on: date) -> RiskAssessment:
        ...
        assessment = RiskAssessment(
            ...
            medicine_score=round(medicine.anomaly_score, 2),
            health_score=round(health.anomaly_score, 2),
            persistence_score=round(persistence, 2),
            geographic_score=round(geographic, 2),
    ...
    def confidence_score(
        self,
        medicine: BaselineComparison,
        health: BaselineComparison,
        persistence: float,
        geographic: float,
    ) -> float:
        supporting_signals = sum(
            [
                medicine.anomaly_score >= 40,
                health.anomaly_score >= 40,
                persistence >= 40,
                geographic >= 40,
            ]
        )
    ...
    def trend_for_area(self, area_id: int, assessed_on: date, current_score: float) -> str:
        previous = self.db.scalar(
            ...
            .where(RiskAssessment.area_id == area_id, RiskAssessment.assessed_on < assessed_on)
            .order_by(RiskAssessment.assessed_on.desc())
        )
        if not previous:
            return "STABLE"
    ...
    def build_explanation(
        ...
        parts = [
            f"Medicine demand changed by {medicine.deviation_percent}% against baseline.",
            f"Health indicators changed by {health.deviation_percent}% against baseline.",
        ]
        if persistence >= 40:
    ...
    def _sum_observations(self, area_id: int, signal_types: set[str], start: date, end: date) -> float:
        return float(
            self.db.scalar(
                select(func.coalesce(func.sum(Observation.value), 0)).where(
                    Observation.area_id == area_id,
                    Observation.signal_type.in_(signal_types),
                    Observation.observed_on >= start,
                    Observation.observed_on <= end,
                    Observation.value >= 0,
                    Observation.data_quality_score >= 0.5,
                )
            )
            or 0
        )
