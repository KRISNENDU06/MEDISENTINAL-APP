...
interface AuthContextType {
  ...
  registerWithOTP: (payload: any) => Promise<boolean>;
...
export const AuthProvider: React.FC<{children:ReactNode}> = ({children}) => {
  ...
  const verifyOTP = async (target:string,otp:string,purpose:'REGISTER'|'RESET_PASSWORD'='REGISTER'):Promise<boolean> => { try { const res = await api.verifyOTP(target,otp,purpose); showToast('success','OTP Verified',res.message); return true; } catch(err:any) { showToast('error','Verification Failed',err.message || 'Invalid OTP code.'); return false; } };
  const registerWithOTP = async (payload:any):Promise<boolean> => { try { const res = await api.registerWithOTP(payload); localStorage.setItem('sih_auth_token',res.access_token); setToken(res.access_token); const profile = await api.getMe(); setUser(profile); showToast('success','Account Created',`Welcome to MEDISENTINEL, ${profile.full_name}!`); return true; } catch(err:any) { showToast('error','Registration Failed',err.message || 'Could not create account.'); return false; } };
  const resetPasswordWithOTP = async (payload:{target:string;otp:string;new_password:string}):Promise<boolean> => { try { const res = await api.resetPasswordWithOTP(payload); showToast('success','Password Updated',res.message); return true; } catch(err:any) { showToast('error','Reset Failed',err.message || 'Could not update password.'); return false; } };
  ...
  const isProvider = !!user && role === 'HEALTH_OFFICIAL' && user.provider_type !== 'PUBLIC_CITIZEN';
  const isAdmin = role === 'ADMIN' || isProvider;
