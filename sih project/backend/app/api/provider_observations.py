...
class ProviderObservationCreate(BaseModel):
    ...
    # Current professional form
    suspected_cases:float=Field(default=0,ge=0); confirmed_cases:float=Field(default=0,ge=0); lab_positivity_rate:float=Field(default=0,ge=0,le=100)
    fever_presentations:float=Field(default=0,ge=0); respiratory_symptoms:float=Field(default=0,ge=0); gi_symptoms:float=Field(default=0,ge=0)
    opd_visits:float=Field(default=0,ge=0); hospital_admissions:float=Field(default=0,ge=0); emergency_visits:float=Field(default=0,ge=0)
    medicine_demand_level:str=Field(default="NORMAL",max_length=30); vector_activity_level:str=Field(default="LOW",max_length=30); environmental_contamination_level:str=Field(default="NONE",max_length=30)
    additional_observations:str=Field(default="",max_length=1000); action_or_verification_needed:str=Field(default="",max_length=1000); data_quality_score:float=Field(default=.95,ge=0,le=1)
    # Previous form fields accepted for stale browser compatibility.
...
def create_provider_observation(payload:ProviderObservationCreate,db:Annotated[Session,Depends(get_db)],current_user:Annotated[User,Depends(require_roles(Role.ADMIN,Role.HEALTH_OFFICIAL))]):
    ...
    # Exactly one official upload per account, area and reporting date.
    duplicate=db.scalar(select(Observation.id).where(Observation.area_id==area.id,Observation.observed_on==payload.observed_on,Observation.source==source).limit(1))
    if duplicate: raise HTTPException(409,f"Today's information for {area.name} has already been uploaded. Only one upload is allowed per day.")
    ...
    # If the old form is what the browser submits, process it instead of returning 422.
